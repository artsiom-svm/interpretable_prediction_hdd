...
class LSTM_one_to_one(BaseModel):
    def __init__(self, n_feat=22, max_seq=2000, n_lstm=1, lstm_sizes="[5]", fc_sizes="[80]", lstm_dropout=0.2, dropout=0.1, activation='sigmoid'):
        super(LSTM_one_to_one, self).__init__()

        lstm_sizes = ast.literal_eval(lstm_sizes)
        fc_sizes = ast.literal_eval(fc_sizes)

        shape = (max_seq, n_feat)
        Input = keras.Input(shape)

        slices = layers.Lambda(lambda x, i: x[:, :, i: i + 1])
        ...
        for i, fc in enumerate(fc_sizes):
            y = layers.Dense(fc, activation=layers.Activation(
                activation), name="fc_%i" % i)(y)
            y = layers.Dropout(dropout, name="dropout_%i" % i)(y)
        y = layers.Dense(1, activation=layers.Activation(activation))(y)
